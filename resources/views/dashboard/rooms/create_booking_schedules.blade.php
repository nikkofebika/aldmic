					...
					<div class="box-body">
						...
						<form method="POST" action="" autocomplete="off">
							...
							<div class="form-group @error('title') has-error @enderror">
								...
								<input type="text" name="title" class="form-control" required placeholder="Nama event" value="{{ old('title') }}">
								@error('title')
							...
							<div class="row">
								...
								<div class="col-md-6 col-sm-12">
									<div class="form-group @error('room_id') has-error @enderror">
										...
										<select name="room_id" id="room_id" class="form-control select2" required>
											...
											@foreach ($rooms as $r)
											<option value="{{ $r->id }}" {{ $r->id == old('room_id') ? "selected" : "" }}>{{ $r->name }}</option>
											@endForeach
										</select>
										@error('room_id')
										<span class="help-block">{{ $message }}</span>
								...
								<div class="col-md-6 col-sm-12">
									<div class="form-group @error('date') has-error @enderror">
										...
										<input type="text" name="date" class="form-control" value="{{ old('date') }}" required id="myDatepicker" disabled readonly>
										@error('date')
										<span class="help-block">{{ $message }}</span>
										@enderror
									</div>
							...
							<div class="row">
								<div class="col-md-6 col-sm-12">
									<div class="form-group @error('start_hour') has-error @enderror">
										...
										<input type="text" name="start_hour" id="start_hour" class="form-control" required value="{{ old('start_hour') }}">
										@error('start_hour')
										<span class="help-block">{{ $message }}</span>
										@enderror
									</div>
								...
								<div class="col-md-6 col-sm-12">
									<div class="form-group @error('end_hour') has-error @enderror">
										...
										<input type="text" name="end_hour" id="end_hour" class="form-control" required value="{{ old('end_hour') }}">
										@error('end_hour')
										<span class="help-block">{{ $message }}</span>
										@enderror
									</div>
